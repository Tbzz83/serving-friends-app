...
#  name                 = "db-subnet-${module.globals.tag}"
#  virtual_network_name = data.azurerm_virtual_network.app-vnet.name
#  resource_group_name  = data.azurerm_resource_group.education.name
#}
...
module "azurerm_mysql_flexible_server" {
  ...
  # 0.0.0.0 turns on allow all azure resources to access MySQL DB setting in Portal including AKS
  # Allowed ips will be overwritten by 'allow_public_access' in prod so just leave
  allowed_ips = concat(var.allowed_ips_list, ["0.0.0.0"])
  ...
  #app_vnet         = data.azurerm_virtual_network.app-vnet
